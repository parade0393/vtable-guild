...
不做承诺的是**类名之间的 DOM 结构关系**。我们会为性能继续调整 DOM —— 例如 2.5.0 删掉了虚拟滚动
的 per-row `<colgroup>`，宽表 DOM 节点数因此少了 40%。这类调整不会让类名消失，但会让依赖父子
关系的选择器失配。所以写覆盖 CSS 时，两种写法的抗变更能力差别很大：
...
上面「保留旧的覆盖 CSS」一节举的 `.ant-table-thead > tr > th` 正是脆的那一类 —— 它能立刻跑起来，
但如果你有机会顺手改写，优先降成单类选择器。
