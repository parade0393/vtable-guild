...
| `rootAutoHeight`    | `scroll.y: 'auto'` 时叠加在 `root` 上的自动高度布局（填满父容器）。               |
| `wrapperAutoHeight` | `scroll.y: 'auto'` 时叠加在 `wrapper` 上的自动高度布局（吃剩余高度）。            |
...
| ------------- | -------------------------------------------- |
| `trDragging`  | 拖拽中的行 `<tr>`。                          |
| `trDropAbove` | 放置目标行（插入到其上方）的 `<tr>` 指示线。 |
| `trDropBelow` | 放置目标行（插入到其下方）的 `<tr>` 指示线。 |
