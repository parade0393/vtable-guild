...
Tailwind CSS 不是必需依赖。使用 `tailwind3` 或 `tailwind4` 模式时，再安装对应版本的 Tailwind CSS。
...
::: warning 必读：与 ant-design-vue / normalize.css 等 reset 共存（Tailwind CSS 4）
...
- `cssMode`
  选择 `prebuilt`、`tailwind3` 或 `tailwind4` 样式模式
- `classPrefix`
