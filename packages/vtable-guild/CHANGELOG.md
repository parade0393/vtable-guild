...
- [`d401f31`](https://github.com/parade0393/vtable-guild/commit/d401f31fd61e45d1d8ebb998f77571b1f076cad8) Thanks [@parade0393](https://github.com/parade0393)! - VTable 新增列显示、列顺序与行拖拽排序三项能力：
  ...
  - **`TableProps.rowDraggable` + `rowDragEnd` 事件**：原生 HTML5 整行拖拽，拖放结束返回重排后的 dataSource 与 `{ draggedKey, targetKey, orderedKeys }`
    - 受控模式，虚拟滚动下可用
    - `orderedKeys` 在树形数据下按数据顺序递归含子节点
    - `customRow` 返回 `draggable: false` 的行退出拖拽
    - 树形数据采用跨父移动语义：拖到目标行前/后即插入目标行的同级位置，节点自带子树，带环防护
  - **新增 UI slots 与 CSS tokens**：`trDragging` / `trDropAbove` / `trDropBelow` slots，`--vtg-table-row-drop-indicator-color` token（两套预设同步）
...
- [#36](https://github.com/parade0393/vtable-guild/pull/36) [`f43b246`](https://github.com/parade0393/vtable-guild/commit/f43b246e8a950be2698bfdcea88e235613ce839f) Thanks [@parade0393](https://github.com/parade0393)! - fix(table): prebuilt 模式列对齐失效、子组件预设掉样式与固定列表格宽度异常
...
- [#34](https://github.com/parade0393/vtable-guild/pull/34) [`fee8d41`](https://github.com/parade0393/vtable-guild/commit/fee8d4141cea01b09f51d221d2c8c18b388a34d5) Thanks [@parade0393](https://github.com/parade0393)! - fix(table): 修复虚拟滚动滚不到最后一行
...
- [`5be8063`](https://github.com/parade0393/vtable-guild/commit/5be806338a837b2f4bb1640bb155d6703191ae9a) Thanks [@parade0393](https://github.com/parade0393)! - compat: 新增 antdv 兼容类名开关 `compatClass`，迁移时不必重写旧的覆盖 CSS
