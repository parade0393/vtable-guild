...
| 场景                                 | 形式                      | 归属                                                                                   |
| ------------------------------------ | ------------------------- | -------------------------------------------------------------------------------------- |
| 文档 demo 的「在 Playground 中编辑」 | `?demo=<base64url(源码)>` | 我们自己的编码，见 `src/utils/share.ts` 与 `site/.vitepress/theme/components/Demo.vue` |
| Playground 内的「分享链接」          | `#<repl 序列化>`          | `@vue/repl` 的 `store.serialize()`                                                     |

分成两种是为了不依赖 repl 内部的 hash 压缩实现；两者互不冲突，`?demo=` 读到后会立刻从地址栏抹掉。
