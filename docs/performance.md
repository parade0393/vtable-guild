...
1. **vxe-table 是这里唯一实现了横向虚拟化的**：200 列只渲染 11 列。它因此在宽表档全面领先——
   ...
   （本条描述的是**基线时刻**的状态。Round-5 已补上 `virtualColumn`，滚动与排序已追平，挂载仍落后。）
...
### Round-5（2026-08-09）· 横向虚拟化 `virtualColumn`

Round-4 的负结果把「宽表慢在哪」钉死在了 `TableCell` 的组件实例数上，也就是说
要拿耗时收益只能**减少可见单元格数**。本轮做的就是这件事：新增 `virtualColumn`
（严格 opt-in，默认 `false`，对齐 `rowHeight` 的先例），把可见单元格数从
「行 × 总列数」降到「行 × 可视列数」。

**只改表体，完全不动表头。** 表头与表体本来就是两张独立的表、各自独立偏移
（表头用真实 `scrollLeft`，表体靠 `Filler` 的 `marginLeft: -offsetX`），对齐靠的是
**绝对像素位置**而不是「列结构相同」。只要表体用占位单元格补齐总宽，列 j 在两边
就落在同一个位置。代价是表头仍付 N 个 `TableHeaderCell`——但那是一次性成本，
相对表体的 2,400 个只占约 8%。

**列宽靠量表头，不要求用户声明数字宽度。** 表头始终渲染全部 N 列，它本身就是
浏览器算好的精确列宽参照，直接读 `<th>` 的 rect 即可，不必复刻 vxe-table 那套
`width / minWidth / auto / 百分比 → renderWidth` 解析（它表头表体都窗口化，
没有完整布局可参照，只能自己算）。因此我们天然支持 `auto`、百分比与
`table-layout: fixed` 下的余量分配。**表头布局不依赖表体，所以「量表头 → 改表体」
不构成反馈环**——这是整个方案能成立的关键。

改动：新增 `useColumnWindow`（前缀和 + 二分求窗口）与 `useColumnMetrics`（量表头），
`PrefixHeights` 改名 `PrefixSums` 并同时服务行高与列宽两条轴，`TableCell` 新增
`widthOverride`，`TableHeaderCell` 给叶子 `<th>` 打 `data-vtg-leaf-col`。
`TableHeader.tsx` / `ColGroup.tsx` 零改动。

实测（1 万行 × 200 列，production，warmup 1 + 正式 5 轮中位数，同一会话内对照）：

| 指标              | virtualColumn 关 | virtualColumn 开 | vxe-table    |
| ----------------- | ---------------- | ---------------- | ------------ |
| 可视列数          | 200              | **13**           | 11           |
| 连续滚动 longtask | 4,269ms          | **0**            | 0            |
| 排序切换          | 118 / 208        | **32 / 0**       | 18 / 0       |
| 滚动到底          | 0.0 / 161        | **0.0 / 0**      | 0.1 / 0      |
| 首次渲染          | 163 / 257        | 155 / 155        | **50 / 249** |
| DOM 节点数        | 3,659            | **1,415**        | 2,089        |

6 列基准档实测**完全持平**（挂载 10 → 11ms，DOM 节点 167 → 167，可视列数 6 → 6）：
列数不多时窗口覆盖全部列，既没有收益也没有代价。这正是它默认关闭的理由。
...
1. **主目标达成**：连续滚动 longtask 从 4,269ms 到 **0**，与 vxe-table 持平。
   #427 描述的「宽表滚动卡顿」在开启后不再复现。排序切换也从 208ms longtask 归零。

2. **首次渲染基本没动（163 → 155ms），仍比 vxe-table 慢 3.1×。** 这是**设计上的**
   取舍不是缺陷：首帧还没有测量结果，我们**刻意回落到渲染全部列**，量到之后再收窄——
   宁可第一帧贵，也不要按估算宽度定位然后错位。要吃掉这一帧就得自己实现一套宽度
   解析（即 vxe 的 `renderWidth`），那正是本方案用「表头全量渲染」换掉的东西。

3. **本轮的会话内基线（4,269ms）明显高于 Round-4 记录的 2,297ms。** 机器状态不同，
   绝对值不可跨会话比。上表三行是同一会话连续采集的，开/关对照内部有效；
   要和 Round-4 比请重跑那一档。

4. 内存列本轮出现负值（GC 时机导致），已知不可靠，故未列入上表。

5. 与 vxe-table 仍存在的差距（本次不修）：表头不虚拟化；合计行不虚拟化；
