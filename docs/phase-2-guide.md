...
- `@vtable-guild/theme` 提供 Table 的默认主题
...
> 完整的 playground 设置见 Step 11。此处先说明 CSS 引入方式。
...
类型定义是 Step 4–10 所有代码的基础。先定义好 `ThemeConfig`、`SlotProps` 等泛型，后续代码可以直接使用，IDE 也能提供正确的类型提示。
...
## Step 7：`theme/src/index.ts` — 统一导出
...
## Step 8：`core/composables/useTheme.ts` — 三层合并逻辑
...
## Step 9：`core/plugin/index.ts` — Vue 插件
...
## Step 10：`core/src/index.ts` — 更新统一导出
...
构建顺序应为：`core`（先构建，因为 theme 的 tsconfig references 指向 core）→ `theme` → `table` → `vtable-guild`。
...
| `cn`       | class 拼接 + 冲突合并（底层调用 tailwind-merge），返回字符串 | Step 8 中 useTheme 合并三层配置时使用      |
